...
from catdots.library.logger import setup_daemon_logging
from catdots.actions._base import BaseAction
from catdots.constants import VALID_ACTIONS


setup_daemon_logging("CatOsDotfillesActionsDaemon")
logger = logging.getLogger("CatOsDotfillesActionsDaemon")

try:
    import setproctitle

    setproctitle.setproctitle("catdots-actions-d")
...
SOCKET_PATH = RUNTIME_DIR / "catdots-actions.sock"
...
def load_action(name: str, conn) -> type[BaseAction] | None:
    """Import the action 'name' from catdots.actions and return the main action class"""
    ...
    try:
        module = importlib.import_module(f"catdots.actions.{name}")
