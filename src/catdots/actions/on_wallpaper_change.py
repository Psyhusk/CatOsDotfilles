...
from catdots.library.logger import setup_actions_state
from catdots.library.fs_ops import path_lexists
from catdots.library.notify import Notification, notify_send
from catdots.library.command import MatugenImage
from catdots.constants import CATDOTS_STATE_DIR
...
class OnWallpaperChange(BaseAction):
    ...
    def _notify_action_failed(self):
        ...
        try:
            notify_send(
                title="CatOsDotfilles-Actions",
                message=f"action: 'on_wallpaper_change' has failed, check logs at '{CATDOTS_STATE_DIR}'",
                urgency="critical",
            )
        finally:
    ...
    def main(self):
        ...
        try:
            with Notification(
                "CatOsDotfilles - Actions", "Executing post wallpaper change scripts."
            ) as n:
                total_steps = 3
