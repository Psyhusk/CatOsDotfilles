...
from catdots.library.fs_ops import copy, remove
...
class WallpapersInstaller:
    def __init__(self, logger: Logger, dry_run: bool, verbose: bool):
        ...
        self.repo_url = "https://github.com/Psyhusk/CatDotsOS-Wallpapers.git"
    ...
    def install(self) -> bool:
        ...
        with Spinner("Installing wallpapers...", verbose=self.verbose) as spinner:
            try:
                ...
                if not self.dry_run:
                    try:
                        avatar = CATDOTS_DATA_DIR / "avatar.png"
                        avatar_dst = CATDOTS_CONFIG_DIR / ".avatar"
                        copy_success = all(
                            [
                                copy(
                                    CATDOTS_WALLPAPERS_DIR,
