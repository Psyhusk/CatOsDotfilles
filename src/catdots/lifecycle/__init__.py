...
from catdots.library.cli_utils import (
    Spinner,
    print_ascii_art,
    print_subtext,
    print_success,
...
class CatOsDotfillesInitializer:
    def __init__(
        self,
    ...
    def _title(self) -> None:
        print_ascii_art("CatOsDotfilles Installer")
        print_subtext("Welcome to CatOsDotfilles initializer!")
        print_subtext(
            "This setup will copy catdots files to the respective user dirs and apply settings."
        )
        print_newline()
    ...
    def _exit(self, success: bool = False) -> None:
        ...
        if success:
            print_success(
                "CatOsDotfilles initialization completed successfully!",
            ...
            print_error(
                "CatOsDotfilles initialization failed. Please check the logs for details"
            )
            sys.exit(1)
    ...
    def install(self) -> None:
        self.logger.info("CatOsDotfilles Installer initialized")

        if self.validator(self.logger) and not self.dry_run:
            print_warning(
                "CatOsDotfilles is already initialized for the current user!",
        ...
        if not confirm(
            ...
            print_warning(
                "CatOsDotfilles initialization cancelled!",
                details="If you want to initialize CatOsDotfilles, please run 'catdots init'.",
            )
            sleep(1)
            sys.exit(1)

        self.logger.info("Starting CatOsDotfilles initialization process...")
